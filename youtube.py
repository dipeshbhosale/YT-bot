...
def download_youtube_audio(url):
    ...
    try:
        ...
        additional_guidance = (
            ...
            "7. Bot Detection / CAPTCHA: If 'Stderr' (above) mentions needing to 'sign in to confirm you’re not a bot', a CAPTCHA, or similar authentication challenges, YouTube is blocking the direct download.\n"
            "   - As `yt-dlp`'s error message suggests, you may need to use cookies. The most common solutions involve adding arguments like `--cookies-from-browser BROWSER_NAME` (e.g., `chrome`, `firefox`) or `--cookies /path/to/cookies.txt` to the `yt-dlp` command.\n"
            "   - For detailed instructions on how to export and use cookies, refer to the `yt-dlp` documentation links provided in its error output (usually in 'Stderr')."
        )
